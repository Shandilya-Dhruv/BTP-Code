...
def deb(data, param_name):
    timestamp = datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S-%f')  # Added %f for milliseconds
    file_name = f"{timestamp}_{param_name}.txt"

    with open("./debug/SIRST3_DNANet/"+file_name, 'w') as file:
        file.write(convert_to_str(data))
