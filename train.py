...
parser.add_argument("--LESPS_Tepoch", default=1, type=int, help="Initial evolution epoch, default: 50")
parser.add_argument("--LESPS_Tloss", default=100000, type=int, help="Tb in evolution threshold, default: 0.5")
...
parser.add_argument("--LESPS_f", default=1, type=int, help="Evolution frequency, default: 5")
...
parser.add_argument("--nEpochs", type=int, default=1, help="Number of epochs, default: 400")
...
def update_gt_mask(save_pth, thresh_Tb, thresh_k):
    ...
    for idx_iter, (img, gt_mask, gt_mask_update, update_dir, size) in tqdm(enumerate(update_loader)):
        img, gt_mask_update = Variable(img).cpu(), Variable(gt_mask_update).cpu()
        pred = net.forward(img)
