...
class Net(nn.Module):
    ...
    def forward(self, img):
        ...
        if self.model_name=='IAANet':
            ...
            if output is not None:
                ...
                probability_map = probability_map.cuda()
                output = output.cuda()
                mask_maps = mask_maps.cuda()
    ...
    def update_gt(self, pred, gt_masks, thresh_Tb, thresh_k, size):
        ...
        for region in measure.regionprops(label_image, cache=False):
            ...
            nbr_mask = ((F.conv2d(cur_point_mask.cuda(), weight=torch.ones(1,1,background_length,background_length).to('cuda'), stride=1, padding=background_length//2))>0).float()
            targets_mask = ((F.conv2d(cur_point_mask.cuda(), weight=torch.ones(1,1,target_length,target_length).to('cuda'), stride=1, padding=target_length//2))>0).float()
            
            ### Candidate Pixels Extraction
            max_limitation = size[0] * size[1] * 0.0015
            threshold_start = (pred.cuda() * nbr_mask.cuda() ).max()*thresh_Tb
            pred = pred.cuda()
            nbr_mask = nbr_mask.cuda()
            threshold_start = threshold_start.cuda()
            threshold_delta = thresh_k * ((pred * nbr_mask ).max() - threshold_start) * (len(region.coords)/max_limitation).to('cuda')
            ...
            if label_image.max() > 1:
                for num_cur in range(label_image.max()):
                    curr_mask = thresh_mask.cuda() * torch.tensor(label_image == (num_cur + 1)).float().unsqueeze(0).to('cuda')         
                    if (curr_mask * targets_mask).sum() == 0:
                        thresh_mask = thresh_mask - curr_mask
                        
            
            ### Average Weighted Summation
            thresh_mask = thresh_mask.cuda()
            update_gt_masks = update_gt_masks.cuda()
            pred = pred.cuda()
        ...
        update_gt_masks = update_gt_masks.cuda()
        update_gt_masks = torch.max(update_gt_masks, (gt_masks==1).float().cuda())
